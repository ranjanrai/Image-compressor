...
const MAX_BINARY_ITERATIONS = 12;

const RESIZE_STEP = 0.97;
...
let estimate=estimateInitialQuality(
    file,
    targetKB
);

let low=Math.max(0.05,estimate-0.15);

let high=Math.min(0.98,estimate+0.15);
...
const ratio = targetKB / localSize;

let scale = Math.sqrt(ratio);

scale = Math.min(scale, RESIZE_STEP);
...
async function compressBatch(items, onProgress) {
    ...
    async function worker() {
        ...
        while (queue.length && !Compressor.cancelled) {

            const item = queue.shift();

            try {

                output.push(await compressImage(item));
    ...
    for (let i = 0; i < Compressor.parallelJobs; i++) {
